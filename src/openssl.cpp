...
bool ZSignAsset::GenerateCMS(void* pscert, void* pspkey, const string& strCDHashData, const string& strCDHashesPlist, const string& strCodeDirectorySlotSHA1, const string& strAltnateCodeDirectorySlot256, string& strCMSOutput)
{
	...
	// iOS and macOS have different tolerance for custom hash-agility attributes.
	// Keep Apple-specific attributes for iOS signatures, but skip them for macOS.
	bool isMacEntitlements = (NULL != strstr(m_strEntitleData.c_str(), "<key>com.apple.application-identifier</key>"));
	if (!isMacEntitlements) {
		// add plist
